			...
			<div class="col-md-12">
				...
				<div class="widget">
					...
					<div class="widget-head">
						<div class="pull-left">所有教学安排分类</div>
					...
					<div class="widget-content">
						...
						<div class="well">
							<form role="form" action="${ctx }/admin/saveTeachArrangeType" class="form-inline">
						...
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								<c:forEach items="${page.content }" var="teachArrangeType">
									<tr>
										<td>${teachArrangeType.id }</td>
										<td>${teachArrangeType.name }</td>
										<td>${fn:length(teachArrangeType.teachArranges) }</td>
										<td><button	class="btn btn-warning" onclick="modifyType(this)"><span class="glyphicon glyphicon-pencil"></span> 修改
											</button> 
											<a
												href="${ctx }/admin/deleteTeachArrangeType?id=${teachArrangeType.id }"
						...
						<div class="pull-left">
							 <ul class="pagination">
								<c:choose>
									...
									<c:otherwise>
										 <li><a href="${ctx }/admin/findTeachArrangeTypes?pageNumber=${pageNumber-1 }">Prev</a></li>
									</c:otherwise>
								</c:choose>	
								<c:forEach begin="1" end="${page.totalPages }" step="1" var="p">
									<c:choose>
										...
										<c:otherwise>
											 <li><a href="${ctx }/admin/findTeachArrangeTypes?pageNumber=${p }">${p }</a></li>
										</c:otherwise>
									</c:choose>	
								</c:forEach>
								<c:choose>
									...
									<c:otherwise>
										 <li><a href="${ctx }/admin/findTeachArrangeTypes?pageNumber=${pageNumber+1 }">Next</a></li>
									</c:otherwise>
								</c:choose>					 
		                     </ul>
	                     </div>
	...
	<script type="text/javascript">
		...
		function modifyType(obj){
			...
			if(0 == $input.size()){
				...
				var ss = "<form action=\"${ctx }/admin/saveTeachArrangeType\"><div class=\"input-group\">"
