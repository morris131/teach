			...
			<div class="col-md-12">
				...
				<div class="widget">
					...
					<div class="widget-head">
						<div class="pull-left">所有师资介绍分类</div>
					...
					<div class="widget-content">
						...
						<div class="well">
							<form role="form" action="${ctx }/admin/saveTeacherIntroType" class="form-inline">
						...
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								<c:forEach items="${page.content }" var="teacherIntroType">
									<tr>
										<td>${teacherIntroType.id }</td>
										<td>${teacherIntroType.name }</td>
										...
										<td><button	class="btn btn-warning" onclick="modifyType(this)"><span class="glyphicon glyphicon-pencil"></span> 修改
											...
											<a
												href="${ctx }/admin/deleteTeacherIntroType?id=${teacherIntroType.id }"
						...
						<div class="pull-left">
							 <ul class="pagination">
								<c:choose>
									...
									<c:otherwise>
										 <li><a href="${ctx }/admin/findTeacherIntroTypes?pageNumber=${pageNumber-1 }">Prev</a></li>
									</c:otherwise>
								</c:choose>	
								<c:forEach begin="1" end="${page.totalPages }" step="1" var="p">
									<c:choose>
										...
										<c:otherwise>
											 <li><a href="${ctx }/admin/findTeacherIntroTypes?pageNumber=${p }">${p }</a></li>
										</c:otherwise>
									</c:choose>	
								</c:forEach>
								<c:choose>
									...
									<c:otherwise>
										 <li><a href="${ctx }/admin/findTeacherIntroTypes?pageNumber=${pageNumber+1 }">Next</a></li>
									</c:otherwise>
								</c:choose>					 
		                     </ul>
	                     </div>
	...
	<script type="text/javascript">
		...
		function modifyType(obj){
			...
			if(0 == $input.size()){
				...
				var ss = "<form action=\"${ctx }/admin/saveTeacherIntroType\"><div class=\"input-group\">"
